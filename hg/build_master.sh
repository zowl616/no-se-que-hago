...
for n in 01 02 03; do
  ffmpeg -y -hide_banner -loglevel error \
    ...
    -filter_complex "[0:v]scale=1920:1080:flags=lanczos:force_original_aspect_ratio=decrease,pad=1920:1080:(ow-iw)/2:(oh-ih)/2:color=0xF1ECDC,setsar=1[bg];[1:v]format=rgba,colorchannelmixer=aa=0.55[ov];[bg][ov]overlay=0:0:format=auto[v]" \
    -map "[v]" -an -c:v libx264 -preset fast -crf 18 -pix_fmt yuv420p \
    titled/${n}.mp4
  echo "  ✓ titled/${n}.mp4"
done

cat > /tmp/hg_concat.txt <<EOF
...
ffmpeg -y -hide_banner -loglevel error \
  -f concat -safe 0 -i /tmp/hg_concat.txt -an \
  -t 15.000 \
  -vf "fps=30,scale=1920:1080:flags=lanczos,format=yuv420p" \
  -c:v libx264 -preset slow -crf 17 -profile:v high -level 4.2 \
  -movflags +faststart -pix_fmt yuv420p \
  final/honestgreens_launch_15s_1080p_silent.mp4
echo "  ✓ final/honestgreens_launch_15s_1080p_silent.mp4"
...
ffmpeg -y -hide_banner -loglevel error \
  ...
  -movflags +faststart -pix_fmt yuv420p \
  final/honestgreens_launch_15s_1080p_audio.mp4
echo "  ✓ final/honestgreens_launch_15s_1080p_audio.mp4"

ffmpeg -y -hide_banner -loglevel error -ss 1.0 -i final/honestgreens_launch_15s_1080p_silent.mp4 -frames:v 1 -q:v 2 final/poster.jpg
ffmpeg -y -hide_banner -loglevel error -i final/honestgreens_launch_15s_1080p_silent.mp4 -vf "fps=1,scale=480:-1,tile=5x3" -frames:v 1 -q:v 3 final/contact_sheet.jpg
