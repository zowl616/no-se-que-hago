...
stills with a subtle ink-bleed treatment, then save as `starts/0N.png` to feed
into Seedance as `--start-image`. Same pattern as aisc_v2/build_starts.py.
"""
...
def hand_paint(overlay: Image.Image) -> Image.Image:
    ...
    a_soft = a.filter(ImageFilter.GaussianBlur(radius=0.9))
