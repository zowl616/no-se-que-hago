...
ffmpeg -y -hide_banner -loglevel error \
  -f concat -safe 0 -i /tmp/dtmf_concat.txt -an \
  -t 15.000 \
  -vf "fps=30,scale=1920:1080:flags=lanczos,format=yuv420p" \
  -c:v libx264 -preset slow -crf 17 -profile:v high -level 4.2 \
  -movflags +faststart -pix_fmt yuv420p \
  final/dtmf_launch_15s_1080p_silent.mp4
echo "  ✓ final/dtmf_launch_15s_1080p_silent.mp4"
...
ffmpeg -y -hide_banner -loglevel error \
  ...
  -movflags +faststart -pix_fmt yuv420p \
  final/dtmf_launch_15s_1080p_audio.mp4
echo "  ✓ final/dtmf_launch_15s_1080p_audio.mp4"

ffmpeg -y -hide_banner -loglevel error -ss 1.0 -i final/dtmf_launch_15s_1080p_silent.mp4 -frames:v 1 -q:v 2 final/poster.jpg
ffmpeg -y -hide_banner -loglevel error -i final/dtmf_launch_15s_1080p_silent.mp4 -vf "fps=1,scale=480:-1,tile=5x3" -frames:v 1 -q:v 3 final/contact_sheet.jpg
